...
export default function MySpopPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Riwayat SPOP Anda</h1>
          <p className="text-slate-500 mt-1">Daftar semua SPOP yang telah Anda input.</p>
        </div>
        <button
              ...
              {filteredData.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    {item.isVerified ? (
                      ...
                      <div className="flex flex-col">
                        ...
                        {item.catatanVerifikasi && (
                          <span className="text-[10px] text-red-500 mt-1 max-w-[150px] truncate" title={item.catatanVerifikasi}>
                            Catatan: {item.catatanVerifikasi}
                          </span>
                        )}
                      </div>
                    ) : (
                      <span className="inline-flex items-center px-2 py-1 rounded-full text-[10px] font-medium bg-slate-100 text-slate-700">
                        Menunggu
                      </span>
                    )}
                  </td>
                  <td className="p-4 text-right space-x-2">
                    {item.isVerified ? (
                      <a href={`/dashboard/my-spop/${item.id}?view=true`} className="inline-flex items-center justify-center px-3 py-1 bg-blue-100 text-blue-700 text-xs font-medium rounded hover:bg-blue-200 transition-colors">
                        Lihat
                      </a>
                    ) : (
                      <>
                        <a href={`/dashboard/my-spop/${item.id}`} className="inline-flex items-center justify-center px-3 py-1 bg-amber-100 text-amber-700 text-xs font-medium rounded hover:bg-amber-200 transition-colors">
                          Edit
                        </a>
